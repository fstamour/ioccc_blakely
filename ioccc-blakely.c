...
o g(o p, o d) {
	...
	while( *j )
	{
		...
		Z(94){
			...
			f=v;
			v=pow(v,r);
			o(b,[q-1]=r*b,[q-1]*pow(f,r-1)+0*b,[q]);
		}
		w(115,sin,cos);
		w(99,cos,sin);
		Z(120) {
...
void s(){
	int e=0;X=Q*8;f=g (Y*8, L*8 );e=(f-X<0)?1:-1;l=0;
	while((l+=h)<8)	{
		V=Y*8+l*R;
		W=L* 8+l*D;
		X=Q*8+l*U;
	...
	for(f=0; f <= Z*2; f++)
	{
		...
		for(B=-Z ; B<Z ; B++) {
			...
			for(O=-Z;O<Z;O++) {
				...
				_((z)(m * 63)*2 + ((z)((p+1)*6) % 2 + (z)((d+1)*6) % 2 == 1 ? 1 : 0));
				O?l:(_(128),_(B!=Z-1 ? Z : Z+1));
			}
			_(128);
		 }		 
		 _(_(129)-129);
